...
pub const DEFAULT_PORT: u16 = 1444;
...
#[macro_export]
macro_rules! linfo  { ($($a:tt)*) => { $crate::telegram::config::log_info(&format!($($a)*)) }; }
#[macro_export]
macro_rules! lwarn  { ($($a:tt)*) => { $crate::telegram::config::log_warn(&format!($($a)*)) }; }
#[macro_export]
macro_rules! lerror { ($($a:tt)*) => { $crate::telegram::config::log_error(&format!($($a)*)) }; }
#[macro_export]
macro_rules! ldebug { ($($a:tt)*) => { $crate::telegram::config::log_debug(&format!($($a)*)) }; }
