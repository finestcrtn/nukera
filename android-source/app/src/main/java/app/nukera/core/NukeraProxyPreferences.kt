...
import app.nukera.utility.checkIpAndPortInCmd
import app.nukera.utility.getCmdArgs
import app.nukera.utility.getCmdEnable
import app.nukera.utility.getStringNotNull
import app.nukera.utility.shellSplit

sealed interface NukeraProxyPreferences {
    companion object {
        fun fromSharedPreferences(preferences: SharedPreferences, context: Context): NukeraProxyPreferences =
            when (preferences.getCmdEnable()) {
                true -> NukeraProxyCmdPreferences(preferences, context)
                false -> NukeraProxyUIPreferences(preferences)
            }
    }
}

class NukeraProxyCmdPreferences(val args: Array<String>) : NukeraProxyPreferences {
    ...
    companion object {
        private fun parseCmdToArguments(preferences: SharedPreferences, context: Context): Array<String> {
            ...
            val ip = preferences.getStringNotNull("nukera_proxy_ip", "127.0.0.1")
            val port = preferences.getStringNotNull("nukera_proxy_port", "1080")
...
class NukeraProxyUIPreferences(val settings: UISettings = UISettings()) : NukeraProxyPreferences {
