...
import app.nukera.R
import app.nukera.core.NukeraProxyUIPreferences
...
object DiagnosticUtils {
    fun buildReport(context: Context): String {
        ...
        val strategy = if (cmdEnabled) {
            ...
            NukeraProxyUIPreferences(preferences).uiargs.drop(1).joinToString(" ")
        }
        ...
        return buildString {
            ...
            appendLine(value(context, R.string.nukera_proxy_ip_setting, ip))
            appendLine(value(context, R.string.nukera_proxy_port_setting, port))
            ...
            appendLine(value(
                context,
                R.string.test_delay,
                "${preferences.getIntStringNotNull("nukera_proxytest_delay", 1)}${context.getString(R.string.diagnostic_seconds)}"
            ))
            appendLine(value(
                context,
                R.string.test_requests,
                preferences.getIntStringNotNull("nukera_proxytest_requests", 1).toString()
            ))
            appendLine(value(
                context,
                R.string.test_requests_limit,
                preferences.getIntStringNotNull("nukera_proxytest_limit", 20).toString()
            ))
            appendLine(value(
                context,
                R.string.test_timeout,
                "${preferences.getLongStringNotNull("nukera_proxytest_timeout", 5)}${context.getString(R.string.diagnostic_seconds)}"
            ))
            appendLine(value(
                context,
                R.string.test_settings_sni,
                preferences.getStringNotNull("nukera_proxytest_sni", "google.com")
            ))
            appendLine(value(
                context,
                R.string.test_settings_usercommands,
                yesNo(context, preferences.getBoolean("nukera_proxytest_usercommands", false))
            ))
